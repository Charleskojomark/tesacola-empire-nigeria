...
export const metadata = {
  ...
  description:
    "Explore Tesacola's curated editorial collections: The Executive, The Heritage, The Travel, and The Occasion.",
};
    ...
    <div className="bg-black min-h-screen text-white pt-28 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {collections.map((col) => {
            const isDeveloping = col.slug === "travel" || col.slug === "occasion";
            return (
              <Link
                key={col.id}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8 flex flex-col justify-end">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] tracking-[0.25em] uppercase text-[#d6be67] font-light">
                      Tesacola Collection
                    </span>
                    {isDeveloping && (
                      <span className="text-[9px] uppercase tracking-wider text-amber-300 border border-amber-400/30 px-1.5 py-0.5 bg-black/60">
                        In Development
                      </span>
                    )}
                  </div>
                  ...
                  <span className="text-[11px] uppercase tracking-widest text-[#d6be67] mt-4 inline-flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                    {isDeveloping ? "Explore Concept & Enquire &rarr;" : "Explore Curated Pieces &rarr;"}
                  </span>
                </div>
              </Link>
            );
